...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      <button
        className={`fixed top-4 z-50 p-2 bg-gray-800 text-white rounded-lg lg:hidden ${
      ...
      >
        ...
        <button
          onClick={() => setSidebarOpen(false)}
          className={`absolute top-3 ${isRTL ? "left-3" : "right-3"} text-white bg-gray-700 hover:bg-gray-600 p-1.5 rounded-full lg:hidden`}
        >
          <X />
        </button>
            ...
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {!isCollapsed && (
                    <span className={`${isRTL ? "mr-3" : "ml-3"} truncate`}>
                      {item.label}
                    </span>
                  )}
                </Link>
